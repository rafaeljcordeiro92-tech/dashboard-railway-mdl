# VERSAO: RAILWAY_SCHEDULER_MDL_V14_ORDEM_MAIN_SALES_INTERVALOS
import os
import sys
import time
import subprocess
...
log(f"Cobrança/dashboard completo: boot + janelas 07/09/11/13/15/17/19/21, respeitando intervalo mínimo de {COBRANCA_MIN_GAP_MIN} min + fechamento automático no último dia às 23h")
...
while True:
    ...
    # PRIORIDADE 1: MAIN completo nas janelas de cobrança.
        ...
        and (_last_cobranca_run_at is None or (agora - _last_cobranca_run_at).total_seconds() >= COBRANCA_MIN_GAP_MIN * 60)
    ):
        _last_cobranca_slot = _cobranca_key
        _last_cobranca_run_at = agora
