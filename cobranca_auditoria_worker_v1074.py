#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Worker leve V10.74 para auditoria imediata de evidências de cobrança.
    ...
    for idx, att in enumerate(attachments(item), 1):
        ...
        if mime.startswith("image/"):
            ...
            if dup:
                ...
                return {"status": "recusado_ia" if exact else "revisao_master", "confidence": 1.0 if exact else 0.70, "fraude_suspeita": True, "duplicate_type": typ, "motivo": f"{'Arquivo idêntico' if exact else 'Imagem visualmente semelhante'} já usado em {prev.get('cliente','')}, título {prev.get('titulo','')}.", "attachments": processed + [rec], "cost_total_usd": 0.0}
            ...
            if dup:
                typ, prev = dup
                exact = typ == "exact_duplicate"
                return {"status": "recusado_ia" if exact else "revisao_master", "confidence": 1.0 if exact else 0.70, "fraude_suspeita": True, "duplicate_type": typ, "motivo": f"{'Arquivo de áudio idêntico' if exact else 'Transcrição muito semelhante'} já usado em {prev.get('cliente','')}, título {prev.get('titulo','')}.", "attachments": processed + [rec], "cost_total_usd": 0.0}
...
Analise todos os arquivos juntos. Aprove somente quando houver contato identificável, mensagem de cobrança e resposta posterior do cliente. Procure inconsistências, recortes, manipulação, simuladores, nomes/telefones divergentes e conflito entre print e áudio. Uma resposta consciente valida a cobrança mesmo sem promessa de pagamento. Em caso de dúvida ou sinal visual suspeito, use revisao_master.
Retorne somente JSON: {{"status":"aprovado|revisao_master|recusado","confidence":0.0,"contato_compativel":true,"contexto_cobranca":true,"resposta_cliente":true,"audio_visivel_no_print":true,"ordem_temporal_coerente":true,"sinais_manipulacao":[],"suspeita_app_gerador":false,"resposta_cliente_resumida":"...","tipo_resposta":"promessa_pagamento|pedido_boleto_pix|data_pagamento|negativa|duvida|responsavel_terceiro|ciente_sem_promessa|outro","motivo":"..."}}"""
    ...
    if result.get("suspeita_app_gerador") or result.get("sinais_manipulacao"):
        ...
        status = "recusado_ia" if conf >= MIN_CONF else "revisao_master"
    else:
        status = "revisao_master"
