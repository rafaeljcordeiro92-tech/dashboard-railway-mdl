...
for key, vd in snapshot_hoje["vendedores"].items():
    ...
    if key not in meta_mes["vendedores"]:
        faixas = calc_pendente_faixas(vd["pendente"])
    ...
    if "grave_alvo" not in vm:
        faixas_mig = calc_pendente_faixas(vd.get("pendente", 0))
...
for f, fd in snapshot_hoje["filiais"].items():
    ...
    if f not in meta_mes["filiais"]:
        faixas_f = calc_pendente_faixas(fd["pendente"])
    ...
    if "grave_alvo" not in fm:
        faixas_mig_f = calc_pendente_faixas(fd.get("pendente", 0))
...
function renderKPIs(){
  ...
  const prevDate=salesDates.filter(d=>String(d)<String(new Date().toISOString().slice(0,10))).slice(-1)[0]||'';
  ...
  const cards=[
    ...
    ((usuarioAtual?.tipo==='master')
      ? makeKpi('💵 Faturamento total',R((Number(sales.venda_realizado_total||0)+Number(sales.servico_realizado_total||0))),'var(--green-400)','Mercantil + serviços realizado')
      : ''),
    ...
    makeKpi('🧮 Markup total', markupTotal?String(markupTotal.toFixed(2)).replace('.',','):'0,00','var(--amber-400)',`(Mercantil + serviços) / custo total ${R(markupCost||0)}`)
  ];
