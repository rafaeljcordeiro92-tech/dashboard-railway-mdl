...
function painelMsgsVisiveis(){return (MSGS||[]).filter(m=>msgMatchesUser(m) && !isExpiredCampaign(m));}
function painelMsgsPendentes(){return painelMsgsVisiveis().filter(m=>isCampaign(m) || !isReadMsg(m));}
function painelMsgsSignature(){return painelMsgsPendentes().map(m=>String(m.id||m.server_time||m.title||m.body||m.message||'')).join('|');}
function renderPainelAvisosHome(){
  if(!usuarioAtual?.is_viewer || !listSection) return;
  listSection.classList.remove('hidden');
  const arr=painelMsgsVisiveis();
  const hasAny=arr.length>0;
  listSection.innerHTML=`<div class="section-head"><div><h2>📺 Painel de avisos</h2><div class="hint">Avisos globais e mensagens enviadas para o Painel aparecem automaticamente aqui.</div></div></div>${hasAny?renderInboxBanner():'<div class="glass panel msg-banner"><h3 style="margin:0 0 6px">🔔 Avisos</h3><div class="small muted">Nenhum aviso para o Painel no momento.</div></div>'}`;
}
function maybeAutoOpenPainelAvisos(force=false){
  if(!usuarioAtual?.is_viewer) return;
  const pendentes=painelMsgsPendentes();
  if(!pendentes.length) return;
  const sig=painelMsgsSignature();
  const key=`mdl_painel_auto_notice_${currentUserKey()}`;
  let last='';
...
function refreshBell(){const count=(MSGS||[]).filter(m=>msgMatchesUser(m) && !m.hidden_on_master && !isExpiredCampaign(m) && !isReadMsg(m) && !isCampaign(m)).length; const bell=document.getElementById('bellCount'); if(bell) bell.textContent=count; if(usuarioAtual?.is_viewer){renderPainelAvisosHome(); maybeAutoOpenPainelAvisos(false);}}
