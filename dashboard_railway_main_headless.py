# VERSAO: COBRANCA10_V15_DASH_RECEBIMENTOS_RELOGIO_SERVICOS_META
...
# V15: MAIN é exclusivo de cobrança/recebimentos.
# Vendas, serviços, margem, markup e diária são publicados somente pelo dashboard_sales_worker_headless.py.
metas_vendas_info = {"json_path": None, "xlsx_path": None, "dados": {}}
margens_brutas_info = {"json_path": None, "xlsx_path": None, "dados": {}}
print("✅ V15_MAIN_SEM_VENDAS: MAIN não coleta vendas/margens/serviços; worker publica pacote completo")
...
print(f"✅ V15_RECEBIMENTO_MES_COMPLETO: recebimentos por faixa desde {_data_corte_parse.strftime('%d/%m/%Y')} inclusive")
...
_servicos_rel_data = {'empresa':{},'servicos':{},'filiais':{},'vendedores':{},'detalhes':[], 'disabled':'V15_SERVICOS_SOMENTE_META'}
...
function secondsUntilNextSlot(intervalMin){
  const now=new Date();
  const m=now.getMinutes();
  const nextMin=(Math.floor(m/intervalMin)+1)*intervalMin;
  const next=new Date(now);
  next.setSeconds(0,0);
  if(nextMin>=60){next.setHours(now.getHours()+1);next.setMinutes(0);}else{next.setMinutes(nextMin);}
  return Math.max(0, Math.floor((next-now)/1000));
}
function secondsUntilNextCobranca(){
  const hours=[7,9,11,13,15,17,19,21];
  const now=new Date();
  for(let d=0; d<3; d++){
    for(const h of hours){
      const t=new Date(now); t.setDate(now.getDate()+d); t.setHours(h,0,0,0);
      if(t>now) return Math.max(0, Math.floor((t-now)/1000));
    }
  }
  return 0;
}
...
function updateNextUpdateClock(){
  const el=document.getElementById('nextUpdateClock'); if(!el) return;
  const sv=secondsUntilNextSlot(20), cb=secondsUntilNextCobranca();
  const next=Math.min(sv, cb);
  const tipo=sv<=cb?'vendas':'cobrança';
  el.textContent=`⏳ Próxima atualização: ${tipo} em ${fmtCountdown(next)}`;
}
...
function renderKPIs(){
  ...
  const rentPct=Number(RENT_OK?.margem_bruta_pct||0);
  ...
  // V15: cards por tipo de serviço removidos da tela inicial.
  // O total de serviços vem exclusivamente do Controle de Metas SGI.
  ...
  document.getElementById('kpis').innerHTML=cards.join('') + `<div class="glass" style="grid-column:1/-1;padding:10px 14px;display:flex;gap:18px;align-items:center;justify-content:space-between;min-height:46px;flex-wrap:wrap"><div style="font-size:12px;color:#a9b2c7">🕒 Última atualização do dashboard: <strong style="color:#e5e7eb">${esc(latestUpdatedLabel()||'--')}</strong></div><div id="nextUpdateClock" style="font-size:12px;color:#fbbf24;font-weight:900">⏳ Próxima atualização: calculando...</div></div>`; try{updateNextUpdateClock()}catch(e){};
}
...
async function pollSalesLive(){
  try{
    ...
    if(margensWrap){
      ...
      try{ Object.assign(MARGENS_BRUTAS, margensWrap||{}); }catch(_e){}
    }
    // V15: não busca relatorio_servicos_mes_atual.json; serviços oficiais vêm da meta SGI.
...
function renderServicosTab(isViewer=false){
  ...
  const tipoCards = tipos.length ? `
    <div class="glass panel" style="margin-bottom:14px">
      <div class="section-head"><div><h2>🛠️ Serviços por tipo</h2><div class="hint">V15: relatório individual de serviços desativado. Use os cards Serviço · Ouro/FOB do Controle de Metas SGI.</div></div></div>
...
function serviceOfficialOverride(ent,key,row){
  // V15: nunca sobrescrever serviço da meta SGI com relatório individual.
  return null;
...
function renderSalesRows(ent, key, label){
  ...
  // V15: serviços permanecem exatamente como vieram do Controle de Metas SGI.
  ...
  return `<div class="sales-card"><h4>${label}</h4><div class="sales-list">${rows.map(r=>{
    ...
    const title=salesTitleForRow(ent,r,key);
